...
import { type Album } from './UserDef';
import { UserTabs } from '../widgets/UserTabs/UserTabs';
import { AlbumTabs } from '../widgets/UserTabs/UserTabs';
import { Main } from '../widgets/UserTabs/UserTabs';
import { useGetAlbumsByUserIdQuery } from '../entities/api/albumsApi';

export const UserAlbums: React.FC = () => {
    const { userId } = useParams<{ userId: string }>();
    const { data: userAlbums, isLoading, isError } = useGetAlbumsByUserIdQuery(Number(userId));
    if (isLoading) {
    return <div>Загрузка альбомов пользователя...</div>;}

    if (isError) {
        return <div>❌ Ошибка при загрузке альбомов</div>;
    }
